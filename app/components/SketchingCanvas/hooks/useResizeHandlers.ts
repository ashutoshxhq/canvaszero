...
export const useResizeHandlers = ({
  ...
  const handleResizeStart = useCallback((handle: ResizeHandle, e: React.MouseEvent, startResize: (shape: Shape, handle: ResizeHandle, point: Point) => void) => {
    e.stopPropagation();
  ...
  const handleResizeMove = useCallback((
    e: React.MouseEvent,
    isResizing: boolean,
    calculateResize: (shape: Shape, startPoint: Point, currentPoint: Point, keepAspectRatio: boolean) => Partial<Shape>
  ) => {
